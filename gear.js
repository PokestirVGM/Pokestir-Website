(function () {
  ...
  // Products are stored as "Vendor — Name". Also tolerates en dashes, plain
  // hyphens, and "â" (mojibake some spreadsheet exports produce for the em dash).
  ...
  function buildSubtags() {
    ...
    for (const tag of list) {
      ...
      btn.type = "button";
      btn.textContent = tag;
      setPillState(btn, state.subtags.has(tag));
      btn.addEventListener("click", () => {
  ...
  function card(item) {
    ...
    el.innerHTML = `
      ...
      <div class="card-foot">
        ${item.sortedSubtags.map((s) => `<span class="tag">${escapeHTML(s)}</span>`).join("")}
        ${dateHTML}
      </div>`;
  ...
  function appendPage() {
    ...
    if (!slice.length) {
      if (currentPage === 0) gridMessage('No items match the selected filters.');
      return;
    }
  ...
  async function init() {
    ...
    const seen = new Set();
    DB = parseCSV(csvText).filter((item) => {
