...
function renderLatest(released) {
  const tiles = latestReleases(released).map((rel) => {
    const n = (rel.tracklist || []).length;
    const game = gameName(rel);
    const count = rel.type === 'Album' && n ? ` · ${n} tracks` : '';
    const img = rel.artwork
      ? `<img src="${escapeHTML(artAtSize(rel.artwork, 500))}" alt="" width="500" height="500" decoding="async">`
      : '';
...
if (typeof require !== 'undefined' && typeof module !== 'undefined' && require.main === module) {
  ...
  if (check) {
    ...
    console.log(`${files.length - 2} release pages + sitemap.xml + home Latest Releases: ${bad} out of date.` +
  ...
  console.log(`${files.length - 2} release pages + sitemap.xml + home Latest Releases checked, ${written} written.`);
}
